...
func encodeDecode(m image.Image) (image.Image, error) {
	var b bytes.Buffer
	err := Encode(&b, APNG{Frames: []Frame{{Image: m}}})
	if err != nil {
		return nil, err
	}
	return Decode(&b)
...
func TestWriterPaletted(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("plen-%d", tc.plen), func(t *testing.T) {
			...
			// Encode the image
			var b bytes.Buffer
			if err := Encode(&b, APNG{Frames: []Frame{{Image: m0}}}); err != nil {
				t.Error(err)
				return
			}
...
func TestWriterLevels(t *testing.T) {
	...
	if err := (&Encoder{}).Encode(&b1, APNG{Frames: []Frame{{Image: m}}}); err != nil {
		t.Fatal(err)
	}
	noenc := &Encoder{CompressionLevel: NoCompression}
	if err := noenc.Encode(&b2, APNG{Frames: []Frame{{Image: m}}}); err != nil {
		t.Fatal(err)
	}
